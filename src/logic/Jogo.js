export class CampoMinado {
    ...
    revelarPosicao(linha, coluna, atualizarVisualCallback) {
        ...
        if (quadrado.mina) {
            this.exibirPainelFimJogo("perdeu");
            return "perdeu";
        }
        ...
        const ganhou = this.regrasDeJogo();
        if (ganhou) {
            this.exibirPainelFimJogo("ganhou");
            return "ganhou";
        }
    ...
    exibirPainelFimJogo(status) {
        ...
        const tempoFinal = elementoTempo ? elementoTempo.textContent : "000";

        if (!modal) return;

        modal.classList.remove("vitoria", "derrota");

        if (status === "ganhou") {
            modal.classList.add("vitoria");
            if (icone) icone.textContent = "🏆";
            if (titulo) titulo.textContent = "VITÓRIA!";
            if (mensagem) mensagem.innerHTML = `Você limpou o campo em <strong>${tempoFinal}</strong> segundos.`;
        } else if (status === "perdeu") {
            modal.classList.add("derrota");
            if (icone) icone.textContent = "💥";
            if (titulo) titulo.textContent = "FIM DE JOGO";
            if (mensagem) mensagem.textContent = "Você pisou em uma mina terrestre.";
        }
        setTimeout(() => {
